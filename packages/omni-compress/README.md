...
// Target file size — binary-searches quality to stay under 500 KB
const { blob: small, quality } = await compressImage(imageFile, {
  format: 'webp',
  maxSizeMB: 0.5,
});
console.log(`Compressed to ${(small.size / 1024).toFixed(0)} KB at quality ${quality?.toFixed(2)}`);
...
| -------------------- | ------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `format`             | `'webp' \| 'avif' \| 'jpeg' \| 'png' \| 'auto'`                     | `'auto'`    | Target output format                                                                                                                                                                                                                                                         |
| `quality`            | `number`                                                            | `0.8`       | Lossy quality `0.0` – `1.0`. When `maxSizeMB` is set, treated as the upper-bound starting quality for binary search                                                                                                                                                          |
| `maxSizeMB`          | `number`                                                            | —           | Target output size ceiling in MB. Binary-searches over quality (up to 6 passes) to find the highest quality that fits. Lossy formats only (WebP, JPEG, AVIF). PNG ignored. See `result.quality` for the final quality used                                                   |
| `maxWidth`           | `number`                                                            | —           | Max output width in px (aspect ratio preserved, downscale only)                                                                                                                                                                                                              |
| `maxHeight`          | `number`                                                            | —           | Max output height in px (aspect ratio preserved, downscale only)                                                                                                                                                                                                             |
| `minWidth`           | `number`                                                            | —           | Min output width in px — upscales if below this value (aspect ratio preserved)                                                                                                                                                                                               |
| `minHeight`          | `number`                                                            | —           | Min output height in px — upscales if below this value (aspect ratio preserved)                                                                                                                                                                                              |
| `width`              | `number`                                                            | —           | Exact canvas width in px. Use with `resize` to control how the image fits                                                                                                                                                                                                    |
| `height`             | `number`                                                            | —           | Exact canvas height in px. Use with `resize` to control how the image fits                                                                                                                                                                                                   |
| `resize`             | `'contain' \| 'cover' \| 'fill' \| 'inside' \| 'outside' \| 'none'` | `'contain'` | Fitting mode when `width`/`height` are set. `contain`: letterbox (pad). `cover`: crop-fill. `fill`: stretch, ignore AR. `inside`: contain without padding (canvas = scaled size). `outside`: cover without cropping (canvas may exceed target). `none`: draw at current size |
| `position`           | `string`                                                            | `'center'`  | Anchor for `contain` pad and `cover` crop. Accepts `'center'`, `'top'`, `'bottom'`, `'left'`, `'right'`, `'top-left'`, `'top-right'`, `'bottom-left'`, `'bottom-right'` (and compass variants e.g. `'north'`)                                                                |
| `withoutEnlargement` | `boolean`                                                           | `false`     | Never upscale beyond the image's natural size. Applies to `contain`, `cover`, `fill`, `inside`, `outside` modes                                                                                                                                                              |
| `convertTypes`       | `string \| string[]`                                                | `[]`        | MIME types (e.g. `'image/png'`) that are locked to their original format when below `convertSize`                                                                                                                                                                            |
| `convertSize`        | `number`                                                            | `5242880`   | Minimum file size (bytes) above which `convertTypes` conversion applies (default 5 MB)                                                                                                                                                                                       |
| `beforeDraw`         | `(canvas, ctx) => void`                                             | —           | Hook called after canvas creation, before drawing the image. Browser fast path only                                                                                                                                                                                          |
| `drew`               | `(canvas, ctx) => void`                                             | —           | Hook called after drawing, before encoding. Browser fast path only                                                                                                                                                                                                           |
| `checkOrientation`   | `boolean`                                                           | `true`      | Auto-rotate image based on EXIF orientation. Set `false` to preserve raw pixel order                                                                                                                                                                                         |
| `retainExif`         | `boolean`                                                           | `false`     | Re-inject original EXIF into JPEG output. Browser fast path only (JPEG→JPEG)                                                                                                                                                                                                 |
| `preserveMetadata`   | `boolean`                                                           | `false`     | Keep EXIF data in the output (FFmpeg / Node paths)                                                                                                                                                                                                                           |
| `strict`             | `boolean`                                                           | `false`     | Return the original if the compressed output is larger                                                                                                                                                                                                                       |
...
Compresses an audio file via WebCodecs AAC fast path or FFmpeg Wasm heavy path. Opus always uses FFmpeg Wasm to produce a valid Ogg container.
...
interface CompressResult {
  ...
  file: File | null; // File with corrected extension; null when input was a plain Blob
  quality?: number; // Final encoder quality used (set when maxSizeMB binary search ran)
}
```

#### `setDefaults(options) / resetDefaults()`

Set module-level defaults applied to every `compressImage()` call. Per-call options override these.

```typescript
import { setDefaults, resetDefaults } from 'omni-compress';

setDefaults({ quality: 0.75, maxWidth: 1920 });

// Now every compressImage() call uses quality=0.75 and maxWidth=1920 unless overridden
const result = await compressImage(file, { format: 'webp' });

resetDefaults(); // Remove all defaults
```
...
- **Main Thread Path:** Standard web files (< 4MB) using native Fast Paths run directly on the main thread. This eliminates `postMessage` communication latency (~50-150ms), matching the performance of legacy main-thread-only libraries like `compressorjs`.
  - _Note: AVIF uses a lower 512KB threshold due to higher CPU intensity._
...
[**API Docs**](https://dharanish-v.github.io/omni-compress/api/) — full TypeDoc-generated reference for all functions, options, types, and error classes. Auto-regenerated on every release.

### Why omni-compress?

[Why omni-compress](https://dharanish-v.github.io/omni-compress/why) — feature matrix vs 7 competitors, quality benchmarks, architecture overview.

### Migration guides

| From                        | Guide                                                                                                                                             |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `browser-image-compression` | [Migrate from browser-image-compression](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-browser-image-compression.md) |
| `compressorjs`              | [Migrate from compressorjs](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-compressorjs.md)                           |
| `jimp`                      | [Migrate from jimp](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-jimp.md)                                           |
| `lamejs`                    | [Migrate from lamejs](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-lamejs.md)                                       |
| `heic2any`                  | [Migrate from heic2any](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-heic2any.md)                                   |
| `@ffmpeg/ffmpeg`            | [Migrate from @ffmpeg/ffmpeg](https://github.com/dharanish-v/omni-compress/blob/master/docs/migrate-from-ffmpeg.md)                               |
