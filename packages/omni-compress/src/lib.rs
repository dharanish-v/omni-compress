...
pub fn compress_to_flac(
    ...
    flac_stream
        .write(&mut sink)
        .map_err(|_| JsValue::from_str("Failed to write FLAC stream"))?;

    Ok(sink.into_inner())
}

#[wasm_bindgen]
pub fn compress_to_mp3(
    pcm_data: &[f32],
    sample_rate: u32,
    channels: u32,
    bitrate: u32,
    ...
    let frames = encoder
        .encode_interleaved(&i16_samples)
...
mod tests {
    ...
    #[test]
    fn test_compress_audio_mp3_structure() {
        let pcm_data = vec![0.0f32; 44100]; // 1 second of silence
        let result = compress_to_mp3(&pcm_data, 44100, 1, 128);
        assert!(result.is_ok());
        assert!(!result.unwrap().is_empty());
    }

    #[test]
    fn test_compress_audio_flac_structure() {
        let pcm_data = vec![0.0f32; 44100]; // 1 second of silence
        let result = compress_to_flac(&pcm_data, 44100, 1, 16);
        assert!(result.is_ok());
        assert!(!result.unwrap().is_empty());
    }
}
