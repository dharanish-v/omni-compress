...
interface UploadZoneProps {
  ...
  t: { selectFile: string; [key: string]: string };
}
...
export function UploadZone({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <svg
          className={`w-12 h-12 transition-transform duration-500 ${isDragging ? 'scale-110' : 'group-hover:scale-110'}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <div className="text-center">
          <span className="text-xl font-black uppercase tracking-tight block text-[var(--theme-text)]">
            {files.length === 0 ? t.selectFile : `${files.length} × ${t.selectFile}`}
          </span>
        ...
        <div className="border-4 p-4 bg-[var(--theme-bg)] border-[var(--theme-border)] max-h-48 overflow-y-auto scrollbar-thin">
          ...
          <div className="space-y-2 text-[var(--theme-text)]">
            {files.map((f, i) => (
              <div
                key={i}
                className="flex items-center justify-between gap-4 p-2 bg-[var(--theme-card-bg)] border-2 border-[var(--theme-border)] text-xs font-bold group"
              >
